...
2. `lib/agent/agent.ts` (`createAgentRun`) builds a failover-aware `ExecutionEngine` (`lib/agent/failover/`) instead of a single bound provider. The engine runs the Vercel AI SDK v7 `streamText` with `stopWhen: stepCountIs(8)`, a system prompt, and **only the tools for services the user has connected** (`lib/agent/tools.ts` `buildTools` gates each tool by `ctx.connected`).
3. The route consumes `engine.run()` in `runReal`; `sdk` events map through `handleSdkPart` (unchanged part logic) and re-emit a custom **NDJSON event protocol** (`lib/agent/events.ts`: `text`/`step`/`approval`/`tools`/`meta`/`error`/`provider`/`reset`/`usage`/`done`), one JSON object per line. The client (`lib/hooks/use-chat.ts`) parses these directly — we deliberately did **not** adopt `@ai-sdk/react`'s `useChat`, to keep the custom streamed-text + live-steps + approval UI.
4. SDK field names are read defensively (`text`/`textDelta`/`delta`, `input`/`args`, `output`/`result`) for v7 robustness.

### Provider failover & checkpoint resume (`lib/agent/failover/`)
- The LLM is stateless; Relay owns the execution state. `ExecutionState` accumulates each completed step's response messages (`onStepFinish` delta, always tool-call/result balanced) into working memory. On a recoverable provider failure the SAME working memory is handed to the next provider, which continues from the point of failure instead of restarting - completed tool calls are never repeated because they're already in the message history.
- Components: `ProviderManager` (builds the ordered candidate chain from configured+healthy providers by `providerPriority()`; resolves models lazily via `resolveModel`), `FailoverController` (advances the chain, prefers healthy), `RetryPolicy`/`classifyFailure` (429/quota/timeout/5xx/network → fail over; auth → skip provider; 400/validation → fatal), `ProviderHealthMonitor` (globalThis circuit breaker + metrics), `CheckpointManager` (persists snapshots to `agent_checkpoints`, no-ops without DB), `ExecutionEngine` (the async-generator run loop; `streamText` is injectable for tests).
- Failover order is `PROVIDER_FAILOVER_ORDER` (default groq,openrouter,openai,anthropic,google,opensource); per-provider fallback model from `failoverModelId()` in `lib/ai/models.ts`. Failover is live and in-request; the user sees a `provider` status line and "Provider switched..." note. After a schema change here, run `npm run db:push`.

### Per-model usage tracking
- `ExecutionState` splits real AI SDK token usage by model (`modelUsageDeltas`); the chat route calls `incrementModelUsage` after each run. WITH a DB it upserts the `model_usage` table (one row per user/model/calendar-month window, bigint counters incremented in SQL so concurrent runs can't lose counts); WITHOUT a DB it falls back to an in-memory per-process store (`__usageMem` on globalThis) so usage is still visible in dev. Counts are exact.
- Budgets: `lib/ai/limits.ts` - `modelLimit()` (per-model) and `providerLimit()` (per-provider), each override-able via env (`MODEL_TOKEN_LIMITS`/`MODEL_TOKEN_LIMIT_DEFAULT`, `PROVIDER_TOKEN_LIMITS`/`PROVIDER_TOKEN_LIMIT_DEFAULT`). `toModelUsage()`/`toProviderUsage()` derive used/remaining/percent. `GET /api/usage` returns usage grouped BY PROVIDER (every model of each configured provider, plus provider totals).
- UI: usage lives in **Settings** (`components/settings/usage-section.tsx`) - one card per provider showing the provider total against its budget and a per-model breakdown, using the shared `UsageBar`. The composer `ModelPicker` only shows label + description (no usage).
